                ...
                if pic_link and pic_link.get('href'):
                    ...
                    if sku_match:
                        ...
                        if sku_id not in seen_skus:
                            ...
                            is_hot_title = any(keyword.lower() in title_lower for keyword in hot_keywords)
                            if(is_hot_title):
                                continue
...
def main():
    # 关键词配置文件路径
    keywords_config_file = "keywords_config_huawei.json"
    
    # 检查关键词配置文件是否存在
    if not os.path.exists(keywords_config_file):
        print(f"❌ 关键词配置文件不存在: {keywords_config_file}")
        print("💡 请创建 keywords_config_huawei.json 文件")
        return
