...
class JDSKUMonitor:
    def __init__(self, keywords_config_file, cookies_source="browser", cookies_file="cookies.txt", 
        ...
        self.user_data_dirs = user_data_dirs or ["./chrome", "./chrome2", "./chrome3", "./chrome4", "./chrome5", "./chrome6", "./chrome7", "./chrome8"]
        ...
        for i, user_data_dir in enumerate(user_data_dirs, 1):
            ...
            try:
                with sync_playwright() as p:
                    ...
                    page.goto("https://mall.jd.com", timeout=60000, wait_until='domcontentloaded')
...
def main():
    ...
    webhook_urls = [
        ...
        "https://open.feishu.cn/open-apis/bot/v2/hook/a89ace45-615f-4cab-9ae9-c2a889b78a67"
    ]
    ...
    if choice == "1":
        # 使用浏览器方式 - 多个不同的用户数据目录
        user_data_dirs = ["./chrome", "./chrome2", "./chrome3", "./chrome4", "./chrome5", "./chrome6", "./chrome7", "./chrome8"]
