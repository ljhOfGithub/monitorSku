...
class BaiduOCR:
    ...
    def get_current_account(self):
        ...
        accounts = [
            ...
            # (BaiduOCRConfig.API_KEY_BACKUP3, BaiduOCRConfig.SECRET_KEY_BACKUP3)
        ]
...
class DeviceQuery:
    ...
    @staticmethod
    def extract_product_code_apple(text):
        '''从OCR识别结果中提取10-12位大写字母和数字组合的商品唯一码（用于苹果）'''
        
        # 匹配10-12位大写字母和数字
        product_code_pattern = r'商品唯一码[：:\s]*([A-Z0-9\s]{10,20})'
        direct_matches = re.findall(product_code_pattern, text)
        
        if direct_matches:
            logging.info(f"直接匹配到商品唯一码: {direct_matches[0]}")
            raw_code = direct_matches[0]
            # 清理空格和换行符
...
def get_activation_status(brand, device_info):
    ...
    try:
        ...
        elif brand == 'apple_warranty_pro':
            # Apple Warranty Pro: 根据purchase.date判断
            activated = device_info.get('activated', False)
            pre_activated = device_info.get('pre-activated', False)
            purchase_date = device_info.get('estPurchaseDate')
            if not pre_activated and not activated:
                return False, None  # 未激活
            else:
                return True, purchase_date  # 已激活，返回激活日期
                ...
                if image_key:
                    ...
                    if downloaded_path:
                        ...
                        if recognized_text:
                            ...
                            if(brand == 'apple_warranty_pro'):
                                product_code = DeviceQuery.extract_product_code_apple(recognized_text)
                            else:
